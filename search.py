...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        if self.env.costFunc == costFunctions[0]:
            cost = cost + 1
        elif self.env.costFunc == costFunctions[1]:  # cost for StepTurn
            turn_cost = self.computeTurnCost(curNode.action, action)
            cost += 1 + turn_cost
        elif self.env.costFunc == costFunctions[2]:  # cost to keep staying left of grid
            cost += state2[0]
        else: # cost to stay on top half with as well as lowest cost
            cost += 1 + (state2[1] / len(self.map))
    ...
    def computeTurnCost(self, action1, action):
        ...
        # Calculates turncost from the 90 degree rotations
        current_index = directions.index(action1)
        desired_index = directions.index(action)
        turn_cost = abs(desired_index - current_index)

        return min(turn_cost, 4 - turn_cost) * 3
    ...
    def findMinManhattanDist(self, pos):
        ...
        #takes the dirty rooms and uses distance_manhattan() to calculate the minimum distance
        dirty_rooms = [
            thing.location
            for thing in self.map
            if isinstance(thing, Dirt) and hasattr(thing, "location")
        ]

        if not dirty_rooms:
            print("No valid dirty rooms found!")
            return float("inf")

        distances = [distance_manhattan(pos, dirt_pos) for dirt_pos in dirty_rooms]
    ...
    def findMinEuclidDist(self, pos):
        ...
        return min((pos[0] - room[0]) ** 2 + (pos[1] - room[1]) ** 2 for room in dirty_rooms)
...
def depth_first_graph_search(problem):
    ...
    """
    #uses the 3.7 to implement the DFS
    node = Node(problem.initial)
    if problem.goal_test(node.state):
        return node, [tuple(node.state)]

    frontier = [node]  # LIFO queue (stack)
    explored = set()

    while frontier:
        node = frontier.pop()
        explored.add(tuple(node.state))  # Ensure states are hashable (e.g., tuples)
...
def best_first_graph_search(problem, f=None):
    ...
    #3.17 UCS pseudocode and additional code for the A* implementation
    ...
    while frontier:
        ...
        explored.add(tuple(node.state) if isinstance(node.state, list) else node.state)

        for action in problem.actions(node.state):
            child = node.child_node(problem, action)


            child_state = (tuple(child.state) if isinstance(child.state, list) else child.state)
...
def astar_search(problem, h=None):
    ...
    return best_first_graph_search(problem, lambda n: np.float64(n.path_cost) + h(n))
